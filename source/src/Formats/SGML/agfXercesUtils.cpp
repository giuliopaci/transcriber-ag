...
void SGMLagfSAXParse(HandlerBase* handler,
			...
			const string& encoding)
{
	...
	//** changes
	// As SGLM needs a less complete validation than XML,
	// and as we're using an xml parsing for SGML file,
	// let's keep on going on error
	// ==> errors will be handled by fatalError of perser where
	// we stop parsing if needed.
...
agfDOMParse(const string& xml_file,
	...
	try {
		...
		Log::err() << "agfXercesUtils.cc:agfDOMParse():" << endl
