...
{
	cerr << "USAGE: " << progname << " [-h] [-v] [-l <lang>] [-c <conv>] <filename>" << endl;
	cerr << "options :" << endl;
	cerr << "\t-c <conv> : applicable conventions for transcription conversion" << endl;
	cerr << "\t-l <lang> : iso639-2 language code for transcription (overrides any language information found in file)" << endl;
	cerr << "\t-v : print program version " << endl;
...
int main(int argc, char* const argv[])
{
	...
	try {
		...
		if ( format != data.guessFileFormat(filename) ) {
			cerr << "Input file is not in TXT format" << endl;
			return 0;
		}
