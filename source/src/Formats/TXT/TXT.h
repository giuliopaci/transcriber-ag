...
* @ingroup	Formats
*
* TXT format plug-in for loading and saving TXT format files.\n
* This plug-in is used by the AG-LIB API.
*/
class DllExport TXT : public agfio_plugin
{
	public:
		TXT();
		~TXT();
...
//> 	Test methods
		virtual list<AGId>
		pubload(const string& filename, const Id& id = "",
	...
	private:
		/**
		* Load a TXT file, converting it to AG's.
		*
		* @param filename
		...
		*   An optional string.
		*/
		virtual string store(const string& filename, const Id& id, map<string,string>* options = NULL)
		...
		*/
		virtual string store(const string& filename, list<string>* const ids,
									map<string,string>* options = NULL)
									throw (agfio::StoreError)
