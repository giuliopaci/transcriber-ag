...
void USAGE(const char* progname)
{
	...
	Log::err() << "\t-t <threshold> : threshold for high-confidence words " << endl;
...
//	#endif
	...
	while ((c = getopt(argc , argv, "c:l:t:vh")) != -1) {
		switch (c) {
		case 'l':
			options["lang"]=optarg;
			break;
		case 'c':
			...
			break;
		case 't':
			options["do_score_highlight"]=optarg;
			break;
		case 'v':
	...
	try {
		...
		if ( format != data.guessFileFormat(filename) ) {
			Log::err() << "Input file is not in VRXML format" << endl;
			return 0;
		}
		...
		data.setConventions(options["conventions"], "eng", false);
